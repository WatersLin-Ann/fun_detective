...
const NotebookUI = (function() {
  ...
  function renderEvidenceMarks(container) {
    const evidenceList = window._gameEvidence || [];
    const notes = PlayerData.getAllEvidenceNotes();

    container.innerHTML = `
      <div class="space-y-3">
        ${evidenceList.length === 0 ? '<p class="text-stone-500 text-center py-8">暂无证据数据</p>' :
        evidenceList.map(ev => {
  ...
  function renderCharacters(container) {
    const witnesses = window._gameWitnesses || [];
    const notes = PlayerData.getAllCharacterNotes();

    container.innerHTML = `
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ${witnesses.length === 0 ? '<p class="text-stone-500 text-center py-8 col-span-2">暂有人物数据</p>' :
        witnesses.map(w => {
